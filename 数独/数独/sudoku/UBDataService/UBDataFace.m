...
+ (void) getClassInfo:(NSString *)className
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
        ...
        if (!error) {
            ...
            [UBDataModel shareCloudModel].error = error;
            result(flag,userInfoArr);
        }
    }];
}
...
+ (void) userRegesterName:(NSString *)username
    ...
    if (username && password && email) {
        ...
        [user signUpInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
            UBDataModel * model = [UBDataModel shareCloudModel];
            if (succeeded) {
                [UBDataModel shareCloudModel].userInfo = @{@"username":user.username,
            ...
            if(model.status == YES){
                [UBDataFace regesterThumbClass:@"headerImage" image:[UIImage imageNamed:@""] result:^(BOOL status) {
                    result(model.status);
                }];
            }else{
                result(model.status);
    ...
    if (username && password) {
        [AVUser logInWithUsernameInBackground:username password:password block:^(AVUser *user, NSError *error) {
            if (!error) {
                ...
                [UBDataModel shareCloudModel].user = user;
                [UBDataModel shareCloudModel].status = YES;
                
            } else {
                [UBDataModel shareCloudModel].status = NO;
                [UBDataModel shareCloudModel].error = error;
            }
            result([UBDataModel shareCloudModel].status);
        }];
    }
}
...
+ (void) getUserThumbClass:(NSString *)className
    ...
    AVQuery *query = [AVQuery queryWithClassName:className];
    [query whereKey:@"thumbId" equalTo:[UBDataModel shareCloudModel].user.objectId];
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *  objects, NSError * _Nullable error) {
        ...
        if ([objects isKindOfClass:[NSArray class]] && objects.count > 0) {
            ...
            if (file) {
                [UBDataModel shareCloudModel].thumbImg = [NSString stringWithFormat:@"%@",file.url];
                result(YES);
            }else{
                result(NO);
            }
...
+ (void) updataDataClass:(NSString *)className
               updataDic:(NSDictionary *)dic
                  result:(void(^)(BOOL status))result{
    AVObject *todo =[AVObject objectWithClassName:className objectId:[UBDataModel shareCloudModel].objectId];
    // 修改属性
...
+ (void) uploadThumbClass:(NSString *)className
    ...
    [query whereKey:@"thumbId" equalTo:[NSString stringWithFormat:@"%@",[UBDataModel shareCloudModel].objectId]];
...
+ (void) regesterThumbClass:(NSString *)className
    ...
    AVFile *file = [AVFile fileWithData:data];
    AVObject *product = [AVObject objectWithClassName:className];
    [product setObject:[UBDataModel shareCloudModel].objectId forKey:@"thumbId"];
    [product setObject:file forKey:@"thumbImg"];
...
+ (void)logout{
    [AVUser logOut];
    if (![UBDataModel shareCloudModel].status == YES) {
