...
@property (strong , nonatomic) UITextField * valicationLabel;//请输入密码或者验证码
...
- (void)initialize{
    ...
    _valicationLabel = [JYCommonKits initTextfieldViewWithPlaceholder:@"输入验证码" andLabelColor:kBlackColor andLabelFont:16*JYScale_Height andFrame:aFrame andJoinView:nil];
    _valicationLabel.textAlignment = NSTextAlignmentLeft;
    [_valicationLabel addTarget:self action:@selector(textValueChanged) forControlEvents:UIControlEventEditingChanged];
    _valicationLabel.keyboardType = UIKeyboardTypeASCIICapable;
    [self addSubview:_valicationLabel];

    aFrame = [WDLUsefulKitModel setNewFrameWithFrame:aFrame OriX:30*JYScale_Width andOriY:CGRectGetMaxY(aFrame) - 2*JYScale_Height andSizeW:_valicationLabel.width andSizeH:1*JYScale_Height];
...
- (void)loginAction:(UIButton *)sender{
    if ([_phoneLabel.text isEqualToString:@""] || [_valicationLabel.text isEqualToString:@""]) {
        return;
    }
    JYWeakify(self);
    [SMSSDK commitVerificationCode:_valicationLabel.text phoneNumber:_phoneLabel.text zone:@"86" result:^(NSError *error) {
        ...
        if (!error)
        {
            ...
            if (weakSelf.statusCode == 0) {
                ...
                controller.number = weakSelf.valicationLabel.text;
                controller.phone = weakSelf.phoneLabel.text;
...
- (void)textValueChanged{
    if (_phoneLabel.text.length != 0 && _valicationLabel.text.length != 0) {
