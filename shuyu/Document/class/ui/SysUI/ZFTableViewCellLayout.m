...
- (instancetype)initWithData:(ZFTableData *)data {
    ...
    if (self) {
        ...
        min_y = 18;
        min_w = [data.name textSizeWithFont:[UIFont systemFontOfSize:15] limitWidth:min_view_w-2*margin-min_x].width;
        min_h = 15;
        ...
        min_h = 1;
        self.lineViewRect = CGRectMake(min_x, min_y, min_w, min_h);
        
        min_x = margin;
        min_y = CGRectGetMaxY(self.lineViewRect) + margin;
        min_w = min_view_w - 2*margin;
...
- (CGFloat)videoHeight {
    ...
    if (self.isVerticalVideo) {
        videoHeight = [UIScreen mainScreen].bounds.size.width * 0.6 * self.data.height/self.data.width;
    } else {
        videoHeight = [UIScreen mainScreen].bounds.size.width * self.data.height/self.data.width;
    }
    return videoHeight;
}
