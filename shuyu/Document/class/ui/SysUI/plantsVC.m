...
@interface plantsVC ()<JYTableViewDataSource,ABAHeaderTabViewDelegate,JYTableViewDelegate,JYBasicViewControllerDelegate>
{
...
- (void)setupUIWithData:(id)data{
    if (typeArr == nil) {
        titleArr = @[@"首页",@"居家",@"水本",@"水培",@"水生",@"草本",@"观叶",@"观花",@"食用"].mutableCopy;
        typeArr = @[@"553",@"7",@"4",@"3",@"2",@"6",@"5",@"1",@"8"].mutableCopy;
    }
    ...
    tableReqModel.link = plantServerUrl;
    [tableReqModel.parameters setObject:@"0" forKey:@"page"];
    [tableReqModel.parameters setObject:typeArr[0] forKey:@"id"];
    [tableReqModel.parameters setObject:@"get_category_posts" forKey:@"json"];
...
- (void)loadDataSuccess:(id)data withParams:(NSMutableDictionary *)params withUrlString:(NSString *)urlString{
    ...
    if (_bodyView.page <= 1) {
        [_bodyView initArrWithDic:data[@"posts"][@"attachments"] withParams:params withUrlString:urlString];
    }else{
        [_bodyView appendArrWithDic:data[@"posts"][@"attachments"] withParams:params withUrlString:urlString];
    }
