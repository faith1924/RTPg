...
- (void)initialize{
    ...
    _phoneLabel = [JYCommonKits initTextfieldViewWithPlaceholder:@"请输入密码" andLabelColor:kBlackColor andLabelFont:16*JYScale_Height andFrame:aFrame andJoinView:self.view];
...
- (void)loginAction:(UIButton *)sender{
    ...
    if (![_passwordLabel.text isEqualToString:_phoneLabel.text]) {
        [MBProgressHUD showError:@"两次密码输入不一致!"];
        return;
    }
    NSDictionary * dic = @{@"password":_passwordLabel.text};
    [LeanCloudInterface updataDataClass:@"_User" updataDic:dic result:^(BOOL status) {
        //退出登录
